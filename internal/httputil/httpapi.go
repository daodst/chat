...
func MakeLocalAPI(metricsName string, f func(*http.Request) util.JSONResponse) http.Handler {
	h := func(req *http.Request) util.JSONResponse {
		logger := util.GetLogger(req.Context())
		if !strings.HasPrefix(req.RemoteAddr, "[::1]") && !strings.HasPrefix(req.RemoteAddr, "127.0.0.1") {
			logger.Infof("，from：%s,to: %s, !", req.RemoteAddr, req.RequestURI)
			return util.ErrorResponse(errors.New("invalid request"))
		}
		return f(req)
	}
	return MakeInternalAPI(metricsName, h)
}
