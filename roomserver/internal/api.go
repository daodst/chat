...
func (r *RoomserverInternalAPI) PerformInvite(
	...
	res1 := r.OutputProducer.ProduceRoomEvents(req.Event.RoomID(), outputEvents)

	
	reveiverId := req.Event.StateKey()
	senderId := req.Event.Sender()
	auto := false

	auto, err = r.UserAPI.JudgeShouldAutoJoin(ctx, senderId, *reveiverId)
	if err != nil {
		sentry.CaptureException(err)
		auto = false
	}
	...
	if auto {
		go func() {
			...
			joinReq := api.PerformJoinRequest{
				RoomIDOrAlias: req.Event.RoomID(),
				UserID:        *reveiverId,
			}
			joinRes := api.PerformJoinResponse{}
			r.Joiner.PerformJoin(ctx, &joinReq, &joinRes) 
		}()
	}
	// end
	return res1
