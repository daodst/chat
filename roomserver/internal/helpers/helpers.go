...
func QueryLatestEventsAndState(
	...
	//util.GetLogger(ctx).WithField("request.RoomID", request.RoomID).Info("QueryLatestEventsAndState 111111")
	roomInfo, err := db.RoomInfo(ctx, request.RoomID)
	if err != nil {
		return err
	}
	//util.GetLogger(ctx).WithField("roomInfo", roomInfo).Info("QueryLatestEventsAndState 222222")
	if roomInfo == nil || roomInfo.IsStub() {
		response.RoomExists = false
		return nil
	}
	//util.GetLogger(ctx).Info("QueryLatestEventsAndState 333333")
	roomState := state.NewStateResolution(db, roomInfo)
	response.RoomExists = true
	response.RoomVersion = roomInfo.RoomVersion
	//util.GetLogger(ctx).Info("QueryLatestEventsAndState 444444")
	...
	//util.GetLogger(ctx).Info("QueryLatestEventsAndState 555555")
	...
	//util.GetLogger(ctx).Info("QueryLatestEventsAndState 666666")
	stateEvents, err := LoadStateEvents(ctx, db, stateEntries)
	if err != nil {
		return err
	}
	//util.GetLogger(ctx).Info("QueryLatestEventsAndState 777777")
	for _, event := range stateEvents {
