...
func (d *Database) roomInfo(ctx context.Context, txn *sql.Tx, roomID string) (*types.RoomInfo, error) {
	...
	//util.GetLogger(ctx).WithField("roomInfo", roomInfo).Info("roomInfo.1")
	if roomInfo != nil {
		//util.GetLogger(ctx).WithField("roomInfo", roomInfo).Info("roomInfo.2")
		d.Cache.StoreRoomServerRoomID(roomInfo.RoomNID, roomID)
		//util.GetLogger(ctx).WithField("roomInfo", roomInfo).Info("roomInfo.3")
		d.Cache.StoreRoomVersion(roomID, roomInfo.RoomVersion)
	}
	//util.GetLogger(ctx).WithField("roomInfo", roomInfo).Info("roomInfo.4")
	return roomInfo, err
}
