...
// is nil.
func LoginFromJSONReader(ctx context.Context, r io.Reader, useraccountAPI uapi.UserLoginAPI, userAPI UserInternalAPIForLogin, cfg *config.ClientAPI, args ...interface{}) (*Login, LoginCleanupFunc, *util.JSONResponse) {
	...
	case authtypes.LoginTypeXs:
		typ = &LoginTypeXs{
			UserAPI: userAPI,
			Config:  cfg,
		}
	case authtypes.LoginTypeCosmos:
		if len(args) == 0 {
			err := util.JSONResponse{
				Code: http.StatusBadRequest,
				JSON: jsonerror.MissingParam("missing clientUserApi"),
			}
			return nil, nil, &err
		}
		typ = &LoginTypeCosmos{
			UserAPI: args[0].(uapi.ClientUserAPI),
			Config:  cfg,
		}
