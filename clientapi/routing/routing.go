...
func Setup(
	...
	// local/insert/chainData
	publicAPIMux.Handle("/local/insert/chainData",
		httputil.MakeLocalAPI("localInsertChainData", func(req *http.Request) util.JSONResponse {
			return InsertChainData(req, userAPI)
		}),
	).Methods(http.MethodPost, http.MethodOptions)
	// /local/insert/relationInvite
	publicAPIMux.Handle("/local/insert/relationInvite",
		httputil.MakeLocalAPI("localInsertRelationInvite", func(req *http.Request) util.JSONResponse {
			return InsertRelationInvite(req, userAPI)
		}),
	).Methods(http.MethodPost, http.MethodOptions)
	// /local/insert/addressBook
	publicAPIMux.Handle("/local/insert/addressBook",
		httputil.MakeLocalAPI("localInsertAddressBook", func(req *http.Request) util.JSONResponse {
			return InsertAddressBook(req, userAPI)
		}),
	).Methods(http.MethodPost, http.MethodOptions)
	// /local/add/telNumbers
	publicAPIMux.Handle("/local/add/telNumbers",
		httputil.MakeLocalAPI("localAddTelNumbers", func(req *http.Request) util.JSONResponse {
			return AddTelNumbers(req, userAPI)
		}),
	).Methods(http.MethodPost, http.MethodOptions)
	// /local/add/mortgage
	publicAPIMux.Handle("/local/add/mortgage",
		httputil.MakeLocalAPI("localAddMortgage", func(req *http.Request) util.JSONResponse {
			return AddMortgage(req, userAPI)
		}),
	).Methods(http.MethodPost, http.MethodOptions)
	// /local/set/chatFee
	publicAPIMux.Handle("/local/set/chatFee",
		httputil.MakeLocalAPI("localSetChatFee", func(req *http.Request) util.JSONResponse {
			return SetChatFee(req, userAPI)
		}),
	).Methods(http.MethodPost, http.MethodOptions)
	...
	if cfg.Matrix.ServerNotices.Enabled {
		...
		// send_server_noticelocalhost,
		publicAPIMux.Handle("/local/send_server_notice",
			httputil.MakeLocalAPI("localSendServerNotice", func(req *http.Request) util.JSONResponse {
				return SendServerNotice(
					req, &cfg.Matrix.ServerNotices,
					cfg, userAPI, rsAPI, asAPI,
					serverNotificationSender, serverNotificationSender,
	...
	v3mux.Handle("/rooms/{roomID}/send/{eventType}/{txnID}", // API
		httputil.MakeAuthAPI("send_message", userAPI, func(req *http.Request, device *userapi.Device) util.JSONResponse {
			vars, err := httputil.URLDecodeMapValues(mux.Vars(req))
			if err != nil {
				return util.ErrorResponse(err)
	...
	v3mux.Handle("/get/host/of/{nodeName}",
		httputil.MakeExternalAPI("get_host_by_node", func(req *http.Request) util.JSONResponse {
			vars, err := httputil.URLDecodeMapValues(mux.Vars(req))
			if err != nil {
				return util.ErrorResponse(err)
			}
			return GetHostByServerNode(req, userAPI, cfg, vars["nodeName"], asAPI, federation)
		}),
	).Methods(http.MethodGet, http.MethodOptions)
	...
	v3mux.Handle("/voip/turnServer",
		httputil.MakeAuthAPI("turn_server", userAPI, func(req *http.Request, device *userapi.Device) util.JSONResponse {
			...
			return RequestTurnServer(userAPI, req, device, cfg)
		}),
	).Methods(http.MethodGet, http.MethodOptions)
	//TODO  add tmp auth information of user
	v3mux.Handle("/voip/turnAuthTmpAdd",
		httputil.MakeAuthAPI("turn_auth_add", userAPI, func(req *http.Request, device *userapi.Device) util.JSONResponse {
			if r := rateLimits.Limit(req, device); r != nil {
				return *r
			}
			return AddTmpAuthInfo(userAPI, req, device, cfg)
		}),
	).Methods(http.MethodPost, http.MethodOptions)
	//TODO  add tmp auth information of user
	v3mux.Handle("/voip/turnAuthTmpDel",
		httputil.MakeAuthAPI("turn_auth_del", userAPI, func(req *http.Request, device *userapi.Device) util.JSONResponse {
			if r := rateLimits.Limit(req, device); r != nil {
				return *r
			}
			return DelTmpAuthInfo(req, device, cfg)
		}),
	).Methods(http.MethodPost, http.MethodOptions)
	...
	// new features
	v3mux.Handle("/new/query_available_users/by/locals",
		httputil.MakeAuthAPI("query_available_users", userAPI, func(req *http.Request, device *userapi.Device) util.JSONResponse {
			return QueryAvailableUsers(req, device)
		}),
	).Methods(http.MethodPost, http.MethodOptions)
	v3mux.Handle("/new/get/user/by/phone",
		httputil.MakeAuthAPI("get_user_by_phone", userAPI, func(req *http.Request, device *userapi.Device) util.JSONResponse {
			return GetUserByPhone(req, device, userAPI, cfg, asAPI, federation)
		}),
	).Methods(http.MethodPost, http.MethodOptions)
}
