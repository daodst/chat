...
import (
	...
	"github.com/matrix-org/dendrite/internal/types"
	"net/http"
	"time"
...
func InsertChainData(
	...
	if r.Localpart == "" {
		return util.JSONResponse{
			...
			JSON: jsonerror.BadJSON("'localpart' must be supplied."),
		}
	}
	err := profileAPI.InsertChainData(req.Context(), r.Localpart, r.Servername, r.ChatFee, r.MortgageFee)
	if err != nil {
		return util.JSONResponse{
			Code: http.StatusBadRequest,
			JSON: jsonerror.BadJSON("InsertChainData err."),
		}
	}
	return util.JSONResponse{
		Code: http.StatusOK,
...
func InsertRelationInvite(
	...
	var r types.RelationInvite
	if resErr := httputil.UnmarshalJSONRequest(req, &r); resErr != nil {
		return *resErr
	}
	if r.Inviter == "" || r.Invitee == "" {
		return util.JSONResponse{
			Code: http.StatusBadRequest,
			JSON: jsonerror.BadJSON("'inviter' and 'invitee' must be supplied."),
		}
	}
	// TODO ，
	err := profileAPI.InsertRelationInvite(req.Context(), r.Inviter, r.Invitee, r.PresentFee)
	if err != nil {
		return util.JSONResponse{
			Code: http.StatusBadRequest,
			JSON: jsonerror.BadJSON("InsertRelationInvite err."),
		}
	}
	return util.JSONResponse{
		Code: http.StatusOK,
...
func InsertAddressBook(
	...
	r := struct {
		...
		FriendLocalpart []string `json:"friend_localpart"`
	}{}
	if resErr := httputil.UnmarshalJSONRequest(req, &r); resErr != nil {
		return *resErr
	}
	if r.Localpart == "" || len(r.FriendLocalpart) == 0 {
		return util.JSONResponse{
			Code: http.StatusBadRequest,
			JSON: jsonerror.BadJSON("'localpart' and 'friend_localpart' must be supplied."),
		}
	}
	err := profileAPI.InsertAddressBook(req.Context(), r.Localpart, r.FriendLocalpart)
	if err != nil {
		return util.JSONResponse{
			Code: http.StatusBadRequest,
			JSON: jsonerror.BadJSON("InsertAddressBook err."),
		}
	}
	return util.JSONResponse{
		Code: http.StatusOK,
...
func AddTelNumbers(
	...
	if r.Localpart == "" || len(r.TelNumbers) == 0 {
		return util.JSONResponse{
			Code: http.StatusBadRequest,
	...
	// TODO ，
	err := profileAPI.AddTelNumbers(req.Context(), r.Localpart, r.TelNumbers)
	if err != nil {
		return util.JSONResponse{
			Code: http.StatusBadRequest,
			JSON: jsonerror.BadJSON("AddTelNumbers err."),
		}
	}
	return util.JSONResponse{
		Code: http.StatusOK,
...
func AddMortgage(
	...
	// chain data，！
	chainData, err := profileAPI.SelectChainData(req.Context(), r.Localpart)
	if err != nil || chainData == nil || chainData.Localpart == "" {
		return util.JSONResponse{
			Code: http.StatusBadRequest,
			JSON: jsonerror.BadJSON("bad request."),
		}
	}
	
	var currentFee, addFee types2.Int
	currentFee, ok := types2.NewIntFromString(chainData.MortgageFee)
	if !ok {
		return util.JSONResponse{
			Code: http.StatusBadRequest,
	...
	addFee, ok2 := types2.NewIntFromString(r.MortgageFee)
	if !ok2 {
		return util.JSONResponse{
			Code: http.StatusBadRequest,
			JSON: jsonerror.BadJSON(""),
	...
	finalFee := currentFee.Add(addFee).String()
	
	err = profileAPI.SetMortgageFee(req.Context(), r.Localpart, finalFee)
	if err != nil {
		return util.JSONResponse{
			Code: http.StatusBadRequest,
			JSON: jsonerror.BadJSON("！"),
...
func SetChatFee(
	...
	_, ok := types2.NewIntFromString(r.ChatFee)
	if !ok {
		return util.JSONResponse{
			Code: http.StatusBadRequest,
			JSON: jsonerror.BadJSON(""),
	...
	err = profileAPI.SetChatFee(req.Context(), r.Localpart, r.ChatFee)
	if err != nil {
		return util.JSONResponse{
			Code: http.StatusBadRequest,
			JSON: jsonerror.BadJSON("！"),
