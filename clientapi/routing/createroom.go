...
func CreateRoom(
	...
	err = ValidateCreateRoomReq(req.Context(), r, device, profileAPI)
	if err != nil {
		return util.JSONResponse{
...
func ValidateCreateRoomReq(ctx context.Context, r createRoomRequest, device *api.Device, profileAPI api.ClientUserAPI) error {
	for _, invitee := range r.Invite {
		autoJoin, err := profileAPI.JudgeShouldAutoJoin(ctx, device.UserID, invitee)
		if err != nil || !autoJoin {
			return errors.New("user " + invitee + " can not be invited")
		}
	}
	return nil
}
	...
	if len(r.Invite) > 0 {
		...
		for _, invitee := range r.Invite {
			...
			if inviteRes.Error != nil {
				...
				_, domain, err := gomatrixserverlib.SplitID('@', invitee)
				if err != nil {
					util.GetLogger(ctx).WithError(err).Error("Parse invitee ID err")
					return util.JSONResponse{
						Code: http.StatusInternalServerError,
						JSON: jsonerror.InternalServerError(),
					}
				...
				if domain == cfg.Matrix.ServerName {
					...
					autoJoin, err = profileAPI.JudgeShouldAutoJoin(ctx, device.UserID, invitee)
					if err != nil {
						util.GetLogger(ctx).WithError(err).Error("Judge if auto join failed")
						return util.JSONResponse{
							Code: http.StatusInternalServerError,
							JSON: jsonerror.InternalServerError(),
						}
					}
					if cfg.Matrix.Mode == "center" {
						autoJoin = true
					}
					if device.DisplayName == cfg.Matrix.ServerNotices.LocalPart || autoJoin { // admin inviting, process with auto join
						perfromJoinres := userutil.PerformJoinRoomForSpecUser(ctx, roomID, invitee, profileAPI, rsAPI)
						if perfromJoinres.Code != http.StatusOK {
							util.GetLogger(ctx).WithError(err).Error("PerformJoin failed")
							return util.JSONResponse{
								Code: http.StatusInternalServerError,
								JSON: jsonerror.InternalServerError(),
							}
