...
func (a *UserInternalAPI) QueryProfile(ctx context.Context, req *api.QueryProfileRequest, res *api.QueryProfileResponse) error {
	...
	res.TelNumbers = prof.TelNumbers
	res.MortgageFee = prof.MortgageFee
	return nil
}

// CouldBeAutoInvited 
func (a *UserInternalAPI) CouldBeAutoInvited(ctx context.Context, inviter, invitee string) (bool, error) { // params localpart of inviter and invitee
	//prof, err := a.DB.GetProfileChatFeeByLocalpart(ctx, invitee)  // TODO inviter，
	//if err != nil {
	//	if err == sql.ErrNoRows {
	//		return false, errors.New("invitee don't exist")
	//	}
	//	return false, err
	//}
	return false, nil
...
func (a *UserInternalAPI) InsertRelationInvite(ctx context.Context, inviter string, invitee string, presentFee string) (err error) {
	return a.DB.InsertRelationInvite(ctx, inviter, invitee, presentFee)
}
